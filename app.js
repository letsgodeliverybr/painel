...
// é filtrado por loja_id quando currentPerfil==='loja' (a loja só deve ver os
// próprios pedidos na lista/tabela) — sem essa lista separada, um entregador
// ocupado numa OUTRA loja aparecia como disponível (verde) no mapa da loja atual.
let _pedidosAtivosGlobal=[];
...
async function abrirModal(id){
  ...
  if(id==='modal-pedido'){
    ...
    setTimeout(async()=>{
      ...
      // Mapa visível pra ADM/suporte/loja igual, sem diferença (pedido do
      // usuário, 2026-09-06 — reverte a restrição de 2026-09-05, que só
...
async function calcularTaxaAuto(forcado=false){
  ...
  // O aviso "digite com número" é só pra digitação ao vivo (oninput) — não
  // faz sentido bloquear um clique manual em "Recalcular", já que existem
...
// geocodificação quando o autocomplete não encontrou o endereço de
// primeira. calcularTaxaAuto() já faz isso, mas só quando np-lat/np-lng
// ainda estão vazios — limpa os dois antes de chamar, pra garantir que
// SEMPRE geocodifica de novo em vez de reaproveitar coordenadas antigas.
function _npRecalcularTaxas(){
  const latEl=document.getElementById('np-lat'),lngEl=document.getElementById('np-lng');
  if(latEl)latEl.value='';
  if(lngEl)lngEl.value='';
  calcularTaxaAuto(true);
}
...
// 2026-09-19; trocado de painel flutuante pra expansão inline no mesmo
// dia, a pedido do usuário — o flutuante sobrepunha a tela, o pedido era
// o próprio card "abrir" como um accordion, empurrando o conteúdo abaixo,
// mesmo padrão já usado por selectedPedidoId/expandido). Cota Uber e
// iFood em paralelo, mostra o preço assim que cada cotação volta. iFood
// só aparece quando o pedido já existe na plataforma iFood (tem
// ifood_order_id) — módulo Shipping do iFood não serve pra pedido de loja
// própria (confirmado contra a doc oficial: "request iFood delivery
// drivers for orders that already exist on the platform").
let _sobDemandaExpandidoId=null; // id do pedido com a lista inline aberta, ou null
let _sobDemandaCotacoes={}; // {[pedidoId]: {uber:{tipo,preco|motivo}, ifood:{...}}}
const _NOMES_PARCEIRO={uber:'Uber Direct',ifood:'iFood Sobre Demanda'};
...
function _abrirSobDemanda(pedidoId){
  if(_sobDemandaExpandidoId===pedidoId){_sobDemandaExpandidoId=null;renderPedidosLista();return;}
  ...
  _sobDemandaExpandidoId=pedidoId;
  _sobDemandaCotacoes[pedidoId]={uber:{tipo:'cotando'}};
  if(mostrarIfood)_sobDemandaCotacoes[pedidoId].ifood={tipo:'cotando'};
  renderPedidosLista();
  _cotarParceiro('uber',pedidoId);
  if(mostrarIfood)_cotarParceiro('ifood',pedidoId);
}
// Bloco inline mostrado dentro do próprio card (abaixo do endereço,
// empurrando o conteúdo normalmente — não é overlay) enquanto
// _sobDemandaExpandidoId===p.id. Ordem pedida pelo usuário: iFood
// primeiro, Uber depois.
function _htmlSobDemandaInline(p){
  const pedidoId=p.id;
...
function _htmlLinhaParceiro(parceiro,pedidoId,estado){
  ...
  return `<button onclick="event.stopPropagation();_confirmarEntregaParceira('${parceiro}','${pedidoId}')" style="display:flex;justify-content:space-between;align-items:center;width:100%;padding:8px;background:rgba(255,255,255,.03);border:1px solid transparent;border-radius:8px;cursor:pointer;color:#DDD;font-family:Inter,sans-serif;text-align:left" onmouseover="this.style.background='rgba(255,255,255,.08)'" onmouseout="this.style.background='rgba(255,255,255,.03)'"><div style="display:flex;align-items:center;gap:8px">${icone}<span style="font-size:13px;font-weight:600">${nome}</span></div><span style="font-weight:700;color:#4a9eff;font-size:13px">R$ ${estado.preco.toFixed(2)}</span></button>`;
}
async function _cotarParceiro(parceiro,pedidoId){
  ...
  try{
    ...
    if(!r.ok||!j.ok||j.disponivel===false){
      novoEstado={tipo:'indisponivel',motivo:j.motivo};
    }else{
      const preco=parceiro==='uber'?(j.quote?.fee!=null?j.quote.fee/100:null):j.preco;
      novoEstado=preco==null?{tipo:'indisponivel'}:{tipo:'ok',preco};
    }
  }catch(e){
    novoEstado={tipo:'indisponivel'};
  }
  if(!_sobDemandaCotacoes[pedidoId])return; // painel foi fechado enquanto cotava
  _sobDemandaCotacoes[pedidoId][parceiro]=novoEstado;
  if(_sobDemandaExpandidoId===pedidoId)renderPedidosLista();
}
  ...
  document.getElementById('app-body').innerHTML=`
    ...
    <div id="mapa-tabela-col" style="flex:1;display:flex;flex-direction:column;overflow:hidden;height:100%;min-width:0">
      <div id="mapa-container-wrap" class="mapa-container" style="position:relative;height:30px;flex-shrink:0;overflow:hidden">
        ...
        ${currentPerfil==='loja'?`<div id="alerta-fatura-loja" onclick="goTab('faturas')" style="display:none;position:absolute;top:46px;left:50%;transform:translateX(-50%);z-index:1001;width:min(520px,calc(100vw - 32px));padding:10px 18px;background:#1e180a;border:1px solid #eab30833;border-left:4px solid #eab308;border-radius:10px;display:flex;gap:12px;align-items:center;box-shadow:0 8px 32px rgba(0,0,0,.65);font-family:Inter,sans-serif;cursor:pointer">
          ...
          ${motoboy?`<div style="background:var(--surface2);border-radius:8px;padding:10px;margin-bottom:10px">${_sec('🛵 Entregador')}
            <div style="display:flex;align-items:center;gap:8px">
              ...
              <div style="flex:1;min-width:0">
                ...
                ${motoboy.telefone&&(currentPerfil==='adm'||currentPerfil==='suporte')?`<div style="font-size:11px;color:var(--sb-text2)">${motoboy.telefone}</div>`:''}
              </div>
...
function atualizarMarcadores(){
  // Preserva o popup do motoboy aberto (se houver) através do rebuild —
  // sem isso, o refresh de 5s (atualizarTudo/realtimeInterval) recriava
  // TODOS os marcadores do zero e fechava qualquer popup aberto sozinho,
  // mesmo sem o usuário mexer em nada (bug real: popup "sumia sozinho"
  // depois de ~5s). O marker é destruído e recriado de qualquer forma
  // (não dá pra atualizar em place sem reescrever a lógica toda), então
  // só reabrimos o popup no marker novo pro mesmo entregador.
  const _motoboyPopupAbertoId=Object.keys(motoboyMarkers).find(id=>motoboyMarkers[id].isPopupOpen());
  ...
  // ocupado em OUTRA loja apareceria como disponível se checássemos só allPedidos.
  const motoboysFiltrados=_estadoMotoboys===3?[]
    :_estadoMotoboys===1?allMotoboys.filter(e=>e.disponivel&&!_pedidosAtivosGlobal.some(p=>(p.motoboy_id===e.id||p.entregador_id===e.id)&&_statusAtivos.includes(p.status_detalhado||p.status)))
    :_estadoMotoboys===2?allMotoboys.filter(e=>_pedidosAtivosGlobal.some(p=>(p.motoboy_id===e.id||p.entregador_id===e.id)&&_statusAtivos.includes(p.status_detalhado||p.status)))
    :allMotoboys;
  motoboysFiltrados.forEach(e=>{const lat=e.lat,lng=e.lng;if(!lat||!lng)return;const temAtivo=_pedidosAtivosGlobal.some(p=>(p.motoboy_id===e.id||p.entregador_id===e.id)&&_statusAtivos.includes(p.status_detalhado||p.status));const cor=temAtivo?'#EF4444':e.disponivel?'#10B981':'#475569';const corViseira=temAtivo?'#991b1b':e.disponivel?'#065f46':'#1f2937';const _nr=(e.nome||'');const nome=(_nr.includes('@')?_nr.split('@')[0]:_nr.split(' ')[0])||'Moto';const svgHelmet=`<svg width="48" height="48" viewBox="0 0 248 243" xmlns="http://www.w3.org/2000/svg"><circle cx="124" cy="121" r="118" fill="none" stroke="white" stroke-width="2"/><g transform="translate(0,243) scale(0.1,-0.1)" stroke="none"><path fill="${cor}" d="M1375 2020 c322 -78 591 -300 702 -578 19 -48 41 -100 48 -117 22 -51 62 -195 85 -305 12 -58 35 -153 51 -213 46 -167 46 -175 0 -316 -49 -147 -105 -251 -183 -334 l-57 -61 -113 38 c-62 21 -189 67 -283 101 -149 55 -254 88 -440 140 -27 8 -97 24 -155 35 -58 11 -135 27 -172 35 -36 8 -139 18 -227 23 -175 9 -193 15 -205 73 -5 28 -33 86 -133 280 -146 281 -162 550 -48 788 25 51 55 106 66 123 19 26 20 33 9 71 -24 86 -24 84 7 90 15 2 117 28 226 56 327 84 428 101 592 97 100 -3 166 -10 230 -26z"/><path fill="${corViseira}" d="M836 1426 c-150 -65 -39 -337 188 -460 353 -192 863 -329 1121 -301 l67 7 -7 36 c-27 154 -119 530 -133 544 -4 4 -70 12 -147 18 -267 20 -557 62 -711 101 -43 11 -105 27 -137 35 -86 22 -212 32 -241 20z"/></g></svg>`;const icon=L.divIcon({html:`<div style="display:flex;flex-direction:column;align-items:center">${svgHelmet}<div style="background:rgba(0,0,0,0.55);color:white;font-size:10px;font-weight:700;padding:1px 4px;border-radius:4px;margin-top:2px;white-space:nowrap">${nome}</div></div>`,iconSize:[64,64],iconAnchor:[32,64],className:''});const _statusAtivosPop=['aceito','no_local','chegou_no_local','chegou_local','em_rota','chegou_destino','retornando'];const _ped=allPedidos.filter(p=>(p.motoboy_id===e.id||p.entregador_id===e.id)&&_statusAtivosPop.includes(p.status_detalhado||p.status));const _badges=_ped.map(p=>`<span onclick="map.closePopup();_irParaPedido('${p.id}')" style="display:inline-block;background:${getStatusCor(p)};color:#fff;font-size:11px;font-weight:700;padding:4px 10px;border-radius:20px;cursor:pointer;margin:2px 2px 0 0">#${p.numero||p.id?.substring(0,6)}</span>`).join('');const _cpfPopup=e.cpf&&(currentPerfil==='adm'||currentPerfil==='suporte')?`<div style="font-size:11px;color:#374151;margin-bottom:4px">🪪 ${e.cpf}</div>`:'';const _telPopup=e.telefone&&(currentPerfil==='adm'||currentPerfil==='suporte')?`<div style="font-size:11px;color:#374151;margin-bottom:${_ped.length?8:4}px">📞 <a href="https://wa.me/55${e.telefone.replace(/\D/g,'')}" target="_blank" style="color:#25D366;font-weight:600;text-decoration:none">${e.telefone}</a></div>`:'';const _popupHtml=`<div style="font-family:Inter,sans-serif;background:#ffffff;color:#111827;padding:4px;min-width:180px;max-width:260px"><div style="font-weight:800;font-size:14px;color:#111827;margin-bottom:4px">${e.nome||'Motoboy'}</div>${_cpfPopup}${_telPopup}${_ped.length?`<div style="display:flex;flex-wrap:wrap;gap:4px">${_badges}</div>`:''}</div>`;motoboyMarkers[e.id]=L.marker([lat,lng],{icon}).addTo(map).bindPopup(_popupHtml,{maxWidth:280});});
  if(_motoboyPopupAbertoId&&motoboyMarkers[_motoboyPopupAbertoId])motoboyMarkers[_motoboyPopupAbertoId].openPopup();
...
const _motoboyPopup=allMotoboys.find(mb=>mb.id===(p.motoboy_id||p.entregador_id));
const _clienteNome=p.cliente_nome||p.cliente||'';
const _clienteEndereco=p.endereco_entrega||p.endereco||'';
...
async function abrirAlocarMotoboy(pedidoId){
  ...
  // Origem = ponto de coleta do pedido: endereço de coleta customizado (encomenda),
...
async function criarPedido(){
  ...
  _btnsCp.forEach(b=>b.disabled=true);
  try{
    await _criarPedidoInterno();
  } finally {
    _btnsCp.forEach(b=>b.disabled=false);
  }
}
...
async function _criarPedidoInterno(){
  ...
  const plataformaOrigemSel=document.getElementById('np-plataforma-origem')?.value||null;
  const coletaOn=document.getElementById('np-coleta-toggle')?.checked;
...
function _ceoInjectStyles(){
  ...
  style.innerHTML=`
    ...
    .ceo-frase{font-size:15px;color:var(--text);font-weight:600;max-width:540px;line-height:1.45;margin-top:8px;padding-left:14px;border-left:3px solid var(--accent);}
...
// Saudação dinâmica por horário de Brasília — manhã/tarde/noite, cada uma
// com ícone próprio. parseInt de toLocaleTimeString(hour12:false) pode
// retornar "24" à meia-noite em alguns engines; normaliza pra 0.
...
// precisar de cron/coluna nova: é só uma função pura de "que dia é hoje",
...
// sozinho à meia-noite e vale igual no primeiro login do dia.
function _ceoDiaDoAno(){
  const[y,m,d]=_dataHojeBrasilia().split('-').map(Number);
  const ini=Date.UTC(y,0,1),atual=Date.UTC(y,m-1,d);
  return Math.floor((atual-ini)/86400000)+1;
}
...
// — cards admin-only na tela Métricas Let's Go, mesma estrutura pros 4
// (input de valor manual + Salvar + barra de progresso em 4 marcos fixos).
// Valor é preenchido manualmente (não calculado de pedidos/GMV) e
// persistido na tabela chave/valor `configuracoes` (mesmo padrão já usado
// pelo Preço Dinâmico — ver _pdChave/salvarPrecoDinamico), uma linha por
...
function _renderMetaProgresso(valor){
  valor=Number(valor)||0;
  const segmentos=_META_MARCOS.map((m,i)=>{
...
async function _buscarMeta(metaId){
  ...
  const el=document.getElementById(`meta-progresso-${metaId}`);if(!el)return;
  const rows=await db('configuracoes','GET',null,`?chave=eq.${meta.chave}`);
  ...
  el.innerHTML=_renderMetaProgresso(valor);
}
function _buscarTodasMetas(){return Promise.all(_METAS_CONFIG.map(m=>_buscarMeta(m.id)));}
async function _salvarMeta(metaId){
  const meta=_METAS_CONFIG.find(m=>m.id===metaId);if(!meta)return;
  const input=document.getElementById(`meta-valor-${metaId}`);
...
function _renderCredito(){
  ...
  el.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:20px">
      ...
      <div class="stat-card"><div class="stat-label">Total Bônus</div><div class="stat-value" id="sc-tot-b" style="font-size:22px;color:#8b5cf6">—</div></div>
...
async function _scBuscar(){
  ...
  else if(tipo==='bonus')qs+='&tipo=eq.bonus';
  ...
  const totB=data.filter(r=>r.tipo==='bonus').reduce((s,r)=>s+(parseFloat(r.valor)||0),0);
  const saldo=totC+totB-totD;
  const e1=document.getElementById('sc-tot-c'),e2=document.getElementById('sc-tot-d'),e3=document.getElementById('sc-tot-s'),e4=document.getElementById('sc-tot-b');
  ...
  wrap.innerHTML=`<div class="card"><div style="overflow-x:auto"><table>
    ...
    <tbody>${data.map(r=>`<tr>
      ...
      <td style="font-weight:700;color:${r.tipo==='credito'?'#10b981':r.tipo==='bonus'?'#7c3aed':'#ef4444'}">R$ ${(parseFloat(r.valor)||0).toFixed(2)}</td>
...
function _renderFaturaAtualCard(c){
  const {diasAtraso,valorAtualizado}=_calcularJurosMultaFatura(c.valor_total,c._vencYMD);
  const vencida=diasAtraso>=1;
  // Card resumido de propósito: só o valor total já atualizado em
  // destaque, sem o detalhamento linha a linha de multa/juros — esse
  // breakdown completo continua só no modal "Ver Fatura"
  // (verFaturaCobranca) e na tabela "Aprovar Faturas" do admin
  // (_buscarCobrancasPendentes), que já usam _calcularJurosMultaFatura
...
async function _iniciarRastreioPublico(pedidoId){
  ...
  L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'© OpenStreetMap contributors',maxZoom:19}).addTo(rMap);
