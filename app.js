...
function atualizarMarcadores(){
  ...
  const _svgPin=`<svg width="32" height="40" viewBox="0 0 100 120" xmlns="http://www.w3.org/2000/svg"><path d="M50,5 C28,5 10,23 10,45 C10,67 50,110 50,110 C50,110 90,67 90,45 C90,23 72,5 50,5 Z" fill="#1A56DB"/><circle cx="50" cy="43" r="16" fill="white"/><ellipse cx="50" cy="112" rx="22" ry="7" fill="none" stroke="#1A56DB" stroke-width="3"/></svg>`;
  lojasVisiveis.forEach(l=>{const lat=l.latitude,lng=l.longitude;if(!lat||!lng)return;const icon=L.divIcon({html:_svgPin,iconSize:[32,40],iconAnchor:[16,40],className:''});lojaMarkers[l.id]=L.marker([lat,lng],{icon}).addTo(map).bindPopup(`<b>${l.nome}</b><br>${l.endereco||'—'}`);});
