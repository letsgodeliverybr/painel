...
async function calcularTaxaAuto(){
  ...
  if(fb)fb.innerHTML=`<span style="color:#22c55e">✅ ${distKm.toFixed(2)} km → Taxa: R$ ${_totalDisplay.toFixed(2)}</span>`;
}
...
async function renderPedidosPage(){
  ...
  document.getElementById('app-body').innerHTML=`<div class="alt-page">
    ...
    ${currentPerfil!=='loja'?`<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:14px">
    ...
    <div class="card"><div style="overflow-x:auto"><table><thead><tr><th>Pedido</th><th>Loja</th><th>Endereço</th><th>Valor</th><th>Entregador</th><th>KM</th><th>Cobrado</th>${currentPerfil!=='loja'?'<th>Pago</th>':''}<th>Status</th><th>Cobrança</th><th>Horário</th></tr></thead><tbody id="tbody-pedidos"><tr><td colspan="${currentPerfil!=='loja'?11:10}" style="text-align:center;padding:32px;color:var(--text3)">Carregando...</td></tr></tbody></table></div></div>
  </div>`;
...
async function _buscarPedidosAdmin(){
  ...
  tbody.innerHTML=`<tr><td colspan="${currentPerfil!=='loja'?11:10}" style="text-align:center;padding:32px;color:var(--text3)">Buscando...</td></tr>`;
