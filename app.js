    ...
    setTimeout(async()=>{
      ...
      modalBody.innerHTML=`
        ...
        <div class="form-row full">
          <div class="fi"><label>Endereço de entrega</label><input id="np-endereco" placeholder="Rua, número, bairro" autocomplete="off" oninput="onChangeEnderecoDebounce()"/><input type="hidden" id="np-lat"/><input type="hidden" id="np-lng"/></div>
        </div>
      ...
      setTimeout(()=>iniciarAutocompleteEndereco('np-endereco','np-lat','np-lng','np-endereco-feedback'),500);
    },80);
  }
  if(id==='modal-loja'){
...
function atualizarMarcadores(){
  ...
  const lojasVisiveis=_estadoLojas===2?[]:_estadoLojas===1?allLojas.filter(l=>allPedidos.some(p=>p.loja_id===l.id)):allLojas;
...
${r1(fi('Endereço',`<input id="el-endereco" value="${v(l.endereco)}" placeholder="Rua, número, bairro" autocomplete="off" style="${is}"/><input type="hidden" id="el-lat" value="${l.latitude||''}"/><input type="hidden" id="el-lng" value="${l.longitude||''}"/>`))}
